...
extension BRAddress: @retroactive CustomStringConvertible, @retroactive Hashable {
	init?(string: String) {
        ...
        UnsafeMutableRawPointer(mutating: &s)
            .assumingMemoryBound(to: CChar.self).update(from: cStr, count: cStr.count)
	}

	init?(scriptPubKey: [UInt8]) {
		self.init()
		guard BRAddressFromScriptPubKey(UnsafeMutableRawPointer(mutating: &s)
	...
	init?(scriptSig: [UInt8]) {
		self.init()
		guard BRAddressFromScriptSig(UnsafeMutableRawPointer(mutating: &s)
            .assumingMemoryBound(to: CChar.self),
	...
	var hash160: UInt160? {
		var hash = UInt160()
		guard BRAddressHash160(&hash, UnsafeRawPointer([s])
