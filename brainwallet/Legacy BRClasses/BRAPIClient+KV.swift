...
private class KVStoreAdaptor: BRRemoteKVStoreAdaptor {
	...
	func ver(key: String, completionFunc: @escaping (UInt64, Date, BRRemoteKVStoreError?) -> Void) {
		...
		client.dataTaskWithRequest(req, authenticated: true, retryCount: 0) { _, resp, err in
			...
			guard let resp = resp, let v = resp.kvVersion, let d = resp.kvDate
			else {
				return completionFunc(0, Date(timeIntervalSince1970: 0), .unknown)
			}
...
private extension HTTPURLResponse {
	var kvDate: Date? {
		if let remDate = allHeaderFields["Last-Modified"] as? String, let dateDate = Date.fromRFC1123(remDate) {
			return dateDate
		}
		return nil
	}
	...
	var kvVersion: UInt64? {
		if let remVer = allHeaderFields["Etag"] as? String, let verInt = UInt64(remVer) {
			return verInt
		}
		return nil
	}
