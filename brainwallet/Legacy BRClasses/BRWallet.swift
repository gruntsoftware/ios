...
class BRWallet {
	...
	init?(transactions: [BRTxRef?], masterPubKey: BRMasterPubKey, listener: BRWalletListener) {
		...
		BRWalletSetCallbacks(cPointer, Unmanaged.passUnretained(self).toOpaque(),
	...
	var receiveAddress: String {
		return charInt8ToString(charArray: BRWalletReceiveAddress(cPointer).s)
	}

	// all previously genereated internal and external addresses
	var allAddresses: [String] {
		var addrs = [BRAddress](repeating: BRAddress(), count: BRWalletAllAddrs(cPointer, nil, 0))
		guard BRWalletAllAddrs(cPointer, &addrs, addrs.count) == addrs.count else { return [] }
	...
	func containsAddress(_ address: String) -> Bool {
		return BRWalletContainsAddress(cPointer, address) != 0
	}

	func addressIsUsed(_ address: String) -> Bool {
		return BRWalletAddressIsUsed(cPointer, address) != 0
	}

	// transactions registered in the wallet, sorted by date, oldest first
	var transactions: [BRTxRef?] {
		var transactions = [BRTxRef?](repeating: nil, count: BRWalletTransactions(cPointer, nil, 0))
		guard BRWalletTransactions(cPointer, &transactions, transactions.count) == transactions.count else { return [] }
		return transactions
	}

	// current wallet balance, not including transactions known to be invalid
	var balance: UInt64 {
		return BRWalletBalance(cPointer)
	}

	// total amount spent from the wallet (exluding change)
	var totalSent: UInt64 {
		return BRWalletTotalSent(cPointer)
	}

	// fee-per-kb of transaction size to use when creating a transaction
	var feePerKb: UInt64 {
		get { return BRWalletFeePerKb(cPointer) }
		set(value) { BRWalletSetFeePerKb(cPointer, value) }
	}

	func feeForTx(amount: UInt64) -> UInt64 {
		return BRWalletFeeForTxAmount(cPointer, amount)
	}

	// returns an unsigned transaction that sends the specified amount from the wallet to the given address
	func createTransaction(forAmount: UInt64, toAddress: String) -> BRTxRef? {
		return BRWalletCreateTransaction(cPointer, forAmount, toAddress)
	}

	// returns an unsigned transaction that sends the specified amount from the wallet to the given address
	func createOpsTransaction(forAmount: UInt64, toAddress: String, opsFee: UInt64, opsAddress: String) -> BRTxRef? {
		return BRWalletCreateOpsTransaction(cPointer, forAmount, toAddress, opsFee, opsAddress)
	}
	...
	func signTransaction(_ tx: BRTxRef, forkId: Int = 0, seed: inout UInt512) -> Bool {
		return BRWalletSignTransaction(cPointer, tx, Int32(forkId), &seed, MemoryLayout<UInt512>.stride) != 0
	}
	...
	func transactionIsValid(_ tx: BRTxRef) -> Bool {
		return BRWalletTransactionIsValid(cPointer, tx) != 0
	}
	...
	func transactionIsPending(_ tx: BRTxRef) -> Bool {
		return BRWalletTransactionIsPending(cPointer, tx) != 0
	}
	...
	func transactionIsVerified(_ tx: BRTxRef) -> Bool {
		return BRWalletTransactionIsVerified(cPointer, tx) != 0
	}
	...
	func amountReceivedFromTx(_ tx: BRTxRef) -> UInt64 {
		return BRWalletAmountReceivedFromTx(cPointer, tx)
	}
	...
	func amountSentByTx(_ tx: BRTxRef) -> UInt64 {
		return BRWalletAmountSentByTx(cPointer, tx)
	}
	...
	func feeForTx(_ tx: BRTxRef) -> UInt64? {
		let fee = BRWalletFeeForTx(cPointer, tx)
	...
	func balanceAfterTx(_ tx: BRTxRef) -> UInt64 {
		return BRWalletBalanceAfterTx(cPointer, tx)
	}

	// fee that will be added for a transaction of the given size in bytes
	func feeForTxSize(_ size: Int) -> UInt64 {
		return BRWalletFeeForTxSize(cPointer, size)
	}
