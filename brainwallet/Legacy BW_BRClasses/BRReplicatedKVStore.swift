...
open class BRReplicatedKVStore: NSObject {
	...
	init(encryptionKey: BRKey, remoteAdaptor: BRRemoteKVStoreAdaptor) throws {
		...
		dbQueue = DispatchQueue(label: "com.gruntsoftware.brainwallet.kvDBQueue", attributes: [])
		super.init()
		try openDatabase()
		try migrateDatabase()
	}
	...
	func syncAllKeys(_ completionHandler: @escaping (BRReplicatedKVStoreError?) -> Void) {
		...
		remote.keys { keyData, err in
			...
			var failures = 0
			let q = DispatchQueue(label: "com.gruntsoftware.brainwallet.kvDBQueue", attributes: DispatchQueue.Attributes.concurrent)
	...
	fileprivate func checkKey(_ key: String) throws {
		let match = keyRegex.matches(in: key, options: [], range: NSRange(location: 0, length: key.count))
		if match.count != 1 {
